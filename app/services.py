...
from .restrictions import is_eligible_for_card, can_purchase_in_country
from .benefits import compute_benefits

def register_client(*, name: str, country: str, monthlyIncome: float, viseClub: bool, cardType: CardType):
    ok, reason = is_eligible_for_card(monthlyIncome=monthlyIncome, viseClub=viseClub, country=country, cardType=cardType)
    if not ok:
